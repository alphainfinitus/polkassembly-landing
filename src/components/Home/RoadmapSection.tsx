...
export default function RoadmapSection() {
	...
	const quarterDiv = (index:number, title: string, list: string[]) => {
		return (
			<div onMouseEnter={() => setActiveElement(index)} key={title} className={`group justify-center min-h-[550px] xl:min-h-[500px] w-full p-6 ease-in-out transition-all transform duration-[700ms] border-r border-r-pa-pink hover:block hover:bg-pa-pink hover:border-r-0 hover:rounded-lg ${activeElement === index ? 'bg-pa-pink border-r-0 rounded-lg block' : 'flex'} ${index === 2 ? 'border-r-0' : ''}`}>
